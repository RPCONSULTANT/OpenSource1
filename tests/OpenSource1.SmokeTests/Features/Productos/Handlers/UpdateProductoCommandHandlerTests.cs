...
public class UpdateProductoCommandHandlerTests
{
    ...
    public async Task Handle_ReturnsNull_WhenProductoDoesNotExist()
    {
        ...
        var response = await handler.Handle(new UpdateProductoCommand(Guid.NewGuid(), "C", "N", null, 1, 1, true), default);
    ...
    {
        var entity = new Producto { Codigo = "OLD", Nombre = "Old", Precio = 1, Stock = 1 };
        var repo = new Mock<IGenericRepository<Producto>>();
        ...
        var handler = new UpdateProductoCommandHandler(unitOfWork.Object);
        var response = await handler.Handle(new UpdateProductoCommand(entity.Id, " NEW ", " Nuevo ", " Desc ", 3.25m, 9, true), default);
