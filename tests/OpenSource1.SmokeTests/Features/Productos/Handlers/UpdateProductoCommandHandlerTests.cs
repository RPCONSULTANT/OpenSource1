...
public class UpdateProductoCommandHandlerTests
{
    ...
    public async Task Handle_ReturnsNull_WhenProductoDoesNotExist()
    {
        ...
        var response = await handler.Handle(new UpdateProductoCommand(Guid.NewGuid(), "C", "N", 1, 1, "cat", "Cat", "UND"), default);
    ...
    {
        var entity = new Producto { Codigo = "OLD", Nombre = "Old", Precio = 1, Stock = 1, Categoria = CategoriaProducto.Of("BASE", "Base"), UnidadMedida = UnidadMedida.Of("UND") };
        var repo = new Mock<IGenericRepository<Producto>>();
        ...
        var handler = new UpdateProductoCommandHandler(unitOfWork.Object);
        var response = await handler.Handle(new UpdateProductoCommand(entity.Id, " NEW ", " Nuevo ", 3.25m, 9, "gen", " General ", "KG"), default);
        ...
        Assert.Equal("General", entity.Categoria.Nombre);
        Assert.Equal("KG", entity.UnidadMedida.Codigo);
